...
3. The users will also set `trustStrimziCaCertificate` field which is only available if the user plan to use TLS based cluster. This property is to specify if user wants to trust the certificates generated by Strimzi or not.
...
```text
if `spec.strimziKafkaRef.trustStrimziCaCertificate` is true:
    use ref to the cert from <cluster_name>-cluster-ca-cert
else if spec.tls.trustAnchorRef defined:  
end if
```
...
| if spec.tls.trustAnchorRef?   | spec.strimziKafkaRef.trustStrimziCaCertificate? | Outcome                                                    |
|-------------------------------|-------------------------------------------------|------------------------------------------------------------|
| provided                      | provided                                        | Use the certificate provided in `spec.tls.trustAnchorRef`. |
| not provided                  | provided                                        | Trust the generated Strimzi CA certificate and use them.   |
| provided                      | not provided                                    | Use the certificate provided in `spec.tls.trustAnchorRef`. |
| not provided                  | not provided                                    | Using public CA.                                           |
